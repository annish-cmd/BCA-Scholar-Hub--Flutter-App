...
class EncryptionService {
  ...
  Future<void> initializeUserKeys() async {
    try {
      ...
      final privateKey = await _secureStorage.read(key: 'private_key_${user.uid}');
      if (privateKey != null) {
      ...
      await _secureStorage.write(
        ...
        value: _rsaAlgorithm.encodePrivateKeyToPem(keyPair.privateKey as RSAPrivateKey),
      );
  ...
  Future<RSAPublicKey?> _getPublicKey() async {
    try {
      ...
      final privateKeyPem = await _secureStorage.read(key: 'private_key_${user.uid}');
    ...
    try {
      ...
      data.forEach((userId, userData) {
        if (userData is Map && userData['key'] is String) {
          try {
            final publicKey = _rsaAlgorithm.parsePublicKeyFromPem(userData['key']);
  ...
  Future<Map<String, dynamic>> encryptGlobalChatMessage(String message) async {
    try {
      ...
      if (encryptedKeys.isEmpty) {
        throw Exception('Failed to encrypt key for any users');
      }
  ...
  Future<String?> decryptGlobalChatMessage(
    ...
    try {
      ...
      try {
        ...
        final decryptedText = AESAlgorithm.decrypt(cipherText, iv, aesKeyBase64);
